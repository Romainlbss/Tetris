...
let test_move_shape(p_param, p_game : t_param * t_game) : unit =
  open_graph(p_param);
...
let test_print_matrix(p_param : t_param) : unit =
...
let test_colision(p_param, p_game : t_param * t_game) : unit =
  let l_cur_shape : t_cur_shape = {position = [|0; p_param.size_y - 1|]; color = red; shape = ref 2}
  in
...
(*let test_final_insert(p_param, p_game : t_param * t_game) : unit =
  let l_cur_shape : t_cur_shape = {position = [|0; 3|]; color = red; shape = ref 3}
  in
    open_graph(p_param);
    draw_shape(l_cur_shape, p_param, p_game);
    for i = 0 to Array.length !(l_cur_shape.shape) - 1
    do
      if p_game.game_space.(
                            l_cur_shape.position.(1) +
                            snd(l_cur_shape.shape.position.(i))).(l_cur_shape.position.(0) +
                            fst(l_cur_shape.shape.position.(i))
                           ) <> 0
