  ...
  let colision : bool ref = ref false in
    ...
    then colision := true
      ...
      do 
        ...
        (
          colision := true; 
          (*print_int(p_game.game_space.(p_cur_shape.position.(1) + snd(p_cur_shape.shape.position.(i))).(p_cur_shape.position.(0) + fst(p_cur_shape.shape.position.(i))));
          print_newline()*)
        );
     done;
  !colision
...
let test_draw_shape() : unit =
  let square:t_shape = {position = [|(0, 0); (1, 0); (0, 1); (1, 1)|]; height = 2; width = 2} in
  let cur_square : t_cur_shape = {position = [|0; 0|]; color = blue; shape = square} in
  let init:t_param = {size_x = 10; size_y = 20; dilat = 25; tick = ref 0.2; border = 20; shapes = [|square|]; colors = [|green; yellow; orange; blue; red; magenta; cyan|]} in
    let game:t_game = { game_space = create_game_space(init.size_y, init.size_x); score = ref 0; main_shape = ref {position = [|0; 0|]; color = blue; shape = square}}
    in
      open_graph(init);
      for i = 0 to 10
      do 
        let shape : t_cur_shape = cur_shape_choice(init) in
        game.main_shape := shape;
        while not (colision(!(game.main_shape), init, game))
        do 
          (
            supp_shape(!(game.main_shape), init, game);
            afficher_matrice(game.game_space);
            print_newline();
            draw_shape(move_down(!(game.main_shape)), init, game);
            afficher_matrice(game.game_space);
            print_newline();
            wait 1;
            game.main_shape := move_down(!(game.main_shape));
            game.main_shape := move_right(!(game.main_shape), init, game);
          )
        done;
      done;
;;
