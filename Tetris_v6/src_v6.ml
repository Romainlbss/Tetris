...
let draw_shape(p_cur_shape, p_param, p_game:t_cur_shape*t_param*t_game):unit = 
  let draw_single_square((x, y): (int*int)):unit = 
    draw_point(x, y, p_param, p_game, p_cur_shape.color)
  in
    for i = 0 to Array.length p_cur_shape.shape.position - 1
    do draw_single_square(add_two_tuple(p_cur_shape.shape.position.(i), (p_cur_shape.position.(0), p_cur_shape.position.(1))));
    done;
;;
  ...
  let shape = p_param.shapes.(Random.int(Array.length p_param.shapes)) in
    let color = p_param.colors.(Random.int(Array.length p_param.colors)) in
      let random_x : int = Random.int((p_param.size_x - shape.width) + 1) in
        {position = [|random_x; p_param.size_y - shape.height|]; color = color; shape = shape}
;;
    ...
    then p_cur_shape
      ...
      (
        for i = 0 to Array.length p_cur_shape.shape.position - 1
          do 
            (
              if p_game.game_space.(p_param.size_y - 1 - snd(p_cur_shape.shape.position.(i)) - p_cur_shape.position.(1)).(p_cur_shape.position.(0) + fst(p_cur_shape.shape.position.(i)) - 1) <> 0
              then left_colision := true;
            )
          done;
          if !left_colision
          then p_cur_shape
...
let move_at_bottom(p_cur_shape, p_param, p_game:t_cur_shape*t_param*t_game): t_cur_shape = 
  ...
  and y = ref p_param.size_y in
    ...
    {position = [|x; p_param.size_y - !y|]; color = p_cur_shape.color; shape = p_cur_shape.shape}
;;

let colision(p_cur_shape, p_param, p_game:t_cur_shape*t_param*t_game): bool =
  let colision : bool ref = ref false in
    if p_cur_shape.position.(1) = 0
    then colision := true
    else 
      for i = 0 to p_cur_shape.shape.width - 1
      do 
        if p_game.game_space.(p_param.size_y - p_cur_shape.position.(1) - snd(p_cur_shape.shape.position.(i))).(p_cur_shape.position.(0) + fst(p_cur_shape.shape.position.(i))) <> 0
        then colision := true;
     done;
  !colision
;;

let supp_shape(p_shape, p_param : t_cur_shape*t_param) : unit =
  set_color(white);
  fill_rect(p_param.border + p_param.dilat * (p_shape.position.(0) + 1) + p_shape.position.(0) + 1,
            p_param.border + p_param.dilat * (p_shape.position.(1) + 1) + p_shape.position.(1) + 1,
            p_shape.shape.width * p_param.dilat + p_shape.shape.width,
            p_shape.shape.height * p_param.dilat + p_shape.shape.height
            )
;;
...
let final_intert(p_shape, p_param, p_game:t_cur_shape*t_param*t_game):unit =
  let nb_color : int ref = ref 0 in
    ...
    for i = 0 to 3
    do p_game.game_space.(p_param.size_y - 1 - snd(p_shape.shape.position.(i)) - p_shape.position.(1)).(p_shape.position.(0) + fst(p_shape.shape.position.(i))) <- !nb_color
    done;
;;
...
let move_shape(key, p_param, p_game:char*t_param*t_game):unit = 
  match key with 
    |'d' -> 
      begin
        supp_shape(!(p_game.main_shape), p_param);
        draw_shape(move_left(!(p_game.main_shape), p_param, p_game), p_param, p_game);
        p_game.main_shape := move_left(!(p_game.main_shape), p_param, p_game);
      end;
    |'h' -> 
      begin
        supp_shape(!(p_game.main_shape), p_param);
        draw_shape(move_right(!(p_game.main_shape), p_param, p_game), p_param, p_game);
        p_game.main_shape := move_right(!(p_game.main_shape), p_param, p_game);
      end;
    |'v' -> 
      begin
        supp_shape(!(p_game.main_shape), p_param);
        draw_shape(move_at_bottom(!(p_game.main_shape), p_param, p_game), p_param, p_game);
...
let drop_shape(p_nb, p_param, p_game:int*t_param*t_game) = 
    ...
    for i = 1 to p_nb do
      let cur_shape : t_cur_shape = cur_shape_choice(p_param) in
        ...
        afficher_matrice(p_game.game_space);
        print_newline();
        wait 1;
...
(*  if p_game.game_space.(p_param.size_y - y - 1).(x) = 0
  then exit 0
  else
  *)
