...
let draw_shape(p_cur_shape, p_param:t_cur_shape*t_param):unit =
  ...
  let draw_single_square((x, y): (int*int)):unit = 
    draw_point((x,y), p_param, p_cur_shape.color)
  in
    for i = 0 to Array.length p_cur_shape.shape.position - 1
    do
      draw_single_square(add_two_tuple(p_cur_shape.shape.position.(i), p_cur_shape.position))
    done
;;
 ...
 *)
let drop_shape(p_shape, p_color, p_nb, p_param,p_play: t_shape*t_color*int*t_param*t_play) = 
  for i = 1 to p_nb do
