...
(*
auteurs: Melodie, Nathan
insert la forme courante dans la matrice de jeu
 *)
let final_intert(p_shape, p_param, p_game : t_cur_shape * t_param * t_game) : unit =
    for i = 0 to 3
    do
      p_game.game_space.(
                         snd(p_param.shapes.(!(p_shape.shape)).position.(i)) +
                         p_shape.position.(1)).(p_shape.position.(0) +
                         fst(p_param.shapes.(!(p_shape.shape)).position.(i))
                        ) <- int_of_color(p_shape.color)
    done
;;
