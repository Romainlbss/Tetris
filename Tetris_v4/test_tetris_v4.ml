...
let test_cur_shape_choice(p_shape,p_color,p_param,p_play: t_shape*t_color*t_param*t_play) : unit =
  open_graph(p_param);
  draw_frame(p_param);
  p_play.main_shape := cur_shape_choice(p_shape,p_color,p_param);
  draw_shape(!(p_play.main_shape),p_param);
  wait(5);
  close_graph()
;;
test_cur_shape_choice(horizontal_bar,red,test_param,test_play);;
