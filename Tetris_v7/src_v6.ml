...
let draw_shape(p_cur_shape, p_param, p_game:t_cur_shape*t_param*t_game):unit = 
  ...
  do p_game.game_space.(p_cur_shape.position.(1) + snd(p_param.shapes.(!(p_cur_shape.shape)).position.(i))).(p_cur_shape.position.(0) + fst(p_param.shapes.(!(p_cur_shape.shape)).position.(i))) <- int_of_color(p_cur_shape.color);
  done;
  affiche_matrice(p_param, p_game);
;;

let cur_shape_choice(p_param:t_param):t_cur_shape = 
  let ind_shape = Random.int(Array.length p_param.shapes) in
    let color = p_param.colors.(Random.int(Array.length p_param.colors)) in
      let random_x : int = Random.int(p_param.size_x - p_param.shapes.(ind_shape).width + 1) in
        {position = [|random_x; 0|]; color = color; shape = ref ind_shape}
;;
  ...
  and right_colision : bool ref = ref false in
    ...
    then p_cur_shape
      ...
      (
        ...
        do 
        (
          if p_game.game_space.(p_cur_shape.position.(1) + i).(p_cur_shape.position.(0) + p_param.shapes.(!(p_cur_shape.shape)).width) <> 0
...
let move_at_bottom(p_cur_shape, p_param, p_game:t_cur_shape*t_param*t_game): t_cur_shape = 
  ...
  and y = ref (p_param.size_y) in
    ...
    {position = [|x; !y - p_param.shapes.(!(p_cur_shape.shape)).height|]; color = p_cur_shape.color; shape = p_cur_shape.shape}
;;
...
let colision(p_cur_shape, p_param, p_game:t_cur_shape*t_param*t_game): bool =
  let colision : bool ref = ref false in
    ...
    then colision := true
      ...
      do 
        if p_game.game_space.(p_cur_shape.position.(1) + snd(p_param.shapes.(!(p_cur_shape.shape)).position.(i)) + p_param.shapes.(!(p_cur_shape.shape)).height).(p_cur_shape.position.(0) + fst(p_param.shapes.(!(p_cur_shape.shape)).position.(i))) <> 0
        then 
        (
          colision := true; 
        );
...
let supp_shape(p_cur_shape, p_param, p_game : t_cur_shape*t_param*t_game) : unit =
  for i = 0 to Array.length (p_param.shapes.(!(p_cur_shape.shape)).position) - 1
  do p_game.game_space.(p_cur_shape.position.(1) + snd(p_param.shapes.(!(p_cur_shape.shape)).position.(i))).(p_cur_shape.position.(0) + fst(p_param.shapes.(!(p_cur_shape.shape)).position.(i))) <- 0;
  done;
  affiche_matrice(p_param, p_game);
;;
...
let final_intert(p_shape, p_param, p_game:t_cur_shape*t_param*t_game):unit =
  let nb_color : int ref = ref 0 in
    ...
    for i = 0 to 3
    do p_game.game_space.(snd(p_param.shapes.(!(p_shape.shape)).position.(i)) + p_shape.position.(1)).(p_shape.position.(0) + fst(p_param.shapes.(!(p_shape.shape)).position.(i))) <- !nb_color
    done;
;;
